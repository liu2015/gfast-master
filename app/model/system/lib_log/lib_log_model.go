...
// arModel is a active record design model for table lib_log operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of lib_log.
	Table = "lib_log"
	// Model is the model object of lib_log.
	Model = &arModel{g.DB("default").Model(Table).Safe()}
	// Columns defines and stores column names for table lib_log.
	Columns = struct {
	
		LibId		string // id 
		Title		string // 脚本名字 
		PosNumber		string // 上传门店编号 
		PosMa		string // 上传门店机台号 
		PosTime		string // 上传时间 
		PosState		string // 是否成功状态 
		PosMsg		string // 脚本运行信息 
	}{
