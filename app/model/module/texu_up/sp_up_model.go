...
// arModel is a active record design model for table sp_up operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of sp_up.
	Table = "sp_up"
	// Model is the model object of sp_up.
	Model = &arModel{g.DB("default").Model(Table).Safe()}
	// Columns defines and stores column names for table sp_up.
	Columns = struct {
	
		Id		string // id 
		ProblemType		string // 问题类型 
		LargeType		string // 大区分类 
		StoreType		string // 反馈门店 
		Describe		string // 问题描述 
		Enclosure		string // 附件 
		Distri		string // 工单分派 
		Labelpub		string // 工单定义补充 
		Other		string // 其他 
	}{
