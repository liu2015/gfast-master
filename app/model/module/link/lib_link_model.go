...
// arModel is a active record design model for table lib_link operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of lib_link.
	Table = "lib_link"
	// Model is the model object of lib_link.
	Model = &arModel{g.DB("default").Model(Table).Safe()}
	// Columns defines and stores column names for table lib_link.
	Columns = struct {
	
		LinkId		string // 列表id 
		LinkName		string // 列表名称 
		LinkUrl		string // 本地运行路径 
		LinkZip		string // 是否需要解压 
		LinkBrief		string // 简介 
		LinkContent		string // 内容 
		LinkEdit		string // 版本 
		LinkDewurl		string // 下载脚本库地址 
		LinkRemak		string // 备注 
		LinkAddtime		string // 添加时间 
		LinkOpen		string // 是否禁用 
	}{
		...
		LinkUrl:		"link_url", 
		LinkZip:		"link_zip", 
		LinkBrief:		"link_brief", 
		LinkContent:		"link_content", 
		LinkEdit:		"link_edit", 
		LinkDewurl:		"link_dewurl", 
		LinkRemak:		"link_remak", 
