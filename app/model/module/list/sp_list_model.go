...
// arModel is a active record design model for table sp_list operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of sp_list.
	Table = "sp_list"
	// Model is the model object of sp_list.
	Model = &arModel{g.DB("default").Model(Table).Safe()}
	// Columns defines and stores column names for table sp_list.
	Columns = struct {
