...
// arModel is a active record design model for table sp_list_work operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of sp_list_work.
	Table = "sp_list_work"
	// Model is the model object of sp_list_work.
	Model = &arModel{g.DB("default").Model(Table).Safe()}
	// Columns defines and stores column names for table sp_list_work.
	Columns = struct {
