...
// arModel is a active record design model for table lib_rary operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of lib_rary.
	Table = "lib_rary"
	// Model is the model object of lib_rary.
	Model = &arModel{g.DB("default").Model(Table).Safe()}
	// Columns defines and stores column names for table lib_rary.
	Columns = struct {
	
		LibId		string // 脚本id 
		LibName		string // 脚本名字 
		LibRul		string // 脚本上传路径 
		LibZip		string // 是否解压 
		LibBrief		string // 简介 
		LibContent		string // 内容 
		LibAddtime		string // 添加时间 
		LibOpen		string // 是否禁用 
		LibEdit		string // 版本 
		LibRemk		string // 备注 
	}{
		...
		LibZip:		"lib_zip", 
		LibBrief:		"lib_brief", 
		LibContent:		"lib_content", 
		LibAddtime:		"lib_addtime", 
		LibOpen:		"lib_open", 
		LibEdit:		"lib_edit", 
		LibRemk:		"lib_remk", 
	}
