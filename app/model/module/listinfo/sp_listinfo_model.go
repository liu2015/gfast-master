...
// arModel is a active record design model for table sp_listinfo operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of sp_listinfo.
	Table = "sp_listinfo"
	// Model is the model object of sp_listinfo.
	Model = &arModel{g.DB("default").Model(Table).Safe()}
	// Columns defines and stores column names for table sp_listinfo.
	Columns = struct {
	
		SpNo		string // 单号 
		SpName		string // 门店问题工单名字 
		ApplyTime		string // 发起时间 
		ApplyerUserid		string // 发起用户id 
		SpRecord		string // 审批详情数组 
		SpRecordSpStatus		string // 审批状态 
		DetailsUserid		string // 审批人id 
		DetailsSpeech		string // 审批意见 
		DetailsSpStatus		string // 分支审批状态2是同意状态 
		DetailsSptime		string // 审批时间 
		ValueProble		string // 问题类型 
		ValueRepair		string // 报修类型 
		ValueRegion		string // 报修区域 
		ValueDescribe		string // 故障描述 
		ValueStore		string // 报修门店 
		ValueLabel		string // 定义标签 
		ValueLabelSup		string // 定义标签补充 
		CommentUserid		string // 备注人id 
		Commenttime		string // 备注人时间 
		Commentcontent		string // 备注问题内容 
		Remarks		string // 备注 
	}{
		...
		ApplyerUserid:		"applyer_userid", 
		SpRecord:		"sp_record", 
		SpRecordSpStatus:		"sp_record_sp_status", 
		DetailsUserid:		"details_userid", 
		DetailsSpeech:		"details_speech", 
		DetailsSpStatus:		"details_sp_status", 
		DetailsSptime:		"details_sptime", 
		ValueProble:		"value_proble", 
		ValueRepair:		"value_repair", 
		ValueRegion:		"value_region", 
		ValueDescribe:		"value_describe", 
		ValueStore:		"value_store", 
		ValueLabel:		"value_label", 
		ValueLabelSup:		"value_label_sup", 
