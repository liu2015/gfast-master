...
type SelectPageReq struct {
	ProblemType string `p:"problem_type"` //问题类型(报修类型)
	Distri      string `p:"distri"`       //工单分派(大区分派)
	Describe    string `p:"describe"`     //问题描述
	StoreType   string `p:"store_type"`   //反馈门店
	Tel         string `p:"tel"`          //联系人电话
	Enclosure   string `p:"enclosure"`    //附件
	Labelpub    string `p:"labelpub"`     //工单定义补充(默认信息填)
	LargeType   string `p:"large_type"`   //备用其他
