...
// arModel is a active record design model for table sp_uplisttest operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of sp_uplisttest.
	Table = "sp_uplisttest"
	// Model is the model object of sp_uplisttest.
	Model = &arModel{g.DB("default").Model(Table).Safe()}
	// Columns defines and stores column names for table sp_uplisttest.
	Columns = struct {
	
		Id		string // id 
		ProblemType		string // 问题类型(报修类型) 
		Distri		string // 工单分派(大区分派) 
		Describe		string // 问题描述 
		StoreType		string // 反馈门店 
		Tel		string // 联系人电话 
		Enclosure		string // 附件 
		Labelpub		string // 工单定义补充(默认信息填) 
		LargeType		string // 备用其他 
		Other		string // 其他 
	}{
